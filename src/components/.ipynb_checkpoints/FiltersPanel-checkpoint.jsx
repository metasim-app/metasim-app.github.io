...
export default function FiltersPanel({ items, filters, onChange, onReset }) {
  ...
  return (
    <Stack
      spacing={3}
      sx={{
        p: 2.5, bgcolor: palette.surface,
        border: `1px solid ${palette.rule}`, borderRadius: 0.75,
      ...
      <FormControlLabel
        control={
          <Checkbox
            ...
            sx={{ color: palette.rule2, '&.Mui-checked': { color: palette.ink }, p: 0.5 }}
          />
        }
          ...
          {facets.map((f) => {
            ...
            return (
              <Stack
                ...
                sx={{
                  cursor: 'pointer', py: 0.4, px: 0.5, borderRadius: 0.5,
                  bgcolor: selected ? palette.surface2 : 'transparent',
                  '&:hover': { bgcolor: palette.surface2 },
                }}
              >
                <Box
                  sx={{
                    width: 8, height: 8, borderRadius: '50%',
                    bgcolor: selected ? palette.ink : palette.rule2, mr: 1,
                  }}
                />
                <Typography
                  variant="body2"
                ...
                <Typography variant="caption" sx={{ fontFamily: 'var(--mono)', color: palette.ink3 }}>
                  {f.count}
                </Typography>
              </Stack>
            )
          ...
          <Stack direction="row" spacing={0.5} flexWrap="wrap" useFlexGap sx={{ mt: 1.5 }}>
            {filters.languages.map((l) => (
              <Chip
                ...
                sx={{
                  bgcolor: palette.surface2, color: palette.ink,
