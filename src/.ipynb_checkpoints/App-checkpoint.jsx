...
export default function App() {
  ...
  return (
    <Box sx={{ minHeight: '100vh', display: 'flex', flexDirection: 'column' }}>
      ...
      {!hasSearched && (
        <HeroPanel mode={mode} busy={busy} onSubmit={runSearch} onModeChange={setMode} />
      )}
        ...
        <Container maxWidth="xl" sx={{ pt: 4, pb: 6, flex: 1 }}>
          ...
          <Grid container spacing={3}>
            ...
            <Grid item xs={12} md={9}>
              ...
              {!busy && error && (
                <ErrorState error={error} onRetry={() => runSearch(submittedQuery, mode)} />
              )}
              {!busy && !error && results && results.length === 0 && (
                <EmptyState
                  title="No similar repositories found."
                  body="The backend returned no matches. Double-check the owner/repo spelling — it must exist in the MetaSim corpus."
                />
              )}
              ...
              {!busy && !error && results && results.length > 0 && filtered.length === 0 && (
                <EmptyState
                  ...
                  body="Loosen the stars or language filters to bring results back."
                />
              )}
            </Grid>
          </Grid>
      ...
      <Drawer
        ...
        PaperProps={{ sx: { width: 320, bgcolor: palette.bg, p: 2 } }}
      >
